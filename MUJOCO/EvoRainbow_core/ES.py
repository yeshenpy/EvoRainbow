...
# from core.Optimizers import Adam, BasicSGD
...
class sepCEM:
    ...
    def __init__(self, num_params, scale,
                 mu_init=None,
                 sigma_init=1e-3,
                 pop_size=256,
                 damp=1e-3,
        ...
        # misc
        self.num_params = num_params    ### 原：policy参数数量, HyperERL：policy表征参数数量
        self.scale = scale
        # distribution parameters
        if mu_init is None:
            self.mu = np.zeros(self.num_params)     ### 全零!!! 根据num_params生成均值mu(policy(embedding)
        else:
            self.mu = np.array(mu_init)
        self.sigma = sigma_init                     ### 给定初始标准sigma值
        self.damp = damp                            ### 噪声大小
        self.damp_limit = damp_limit                ### 噪声scale，指数递减
        self.tau = 0.95
        self.cov = self.sigma * np.ones(self.num_params)    ### 协方差

        # elite stuff
        self.elitism = elitism
        self.elite = np.sqrt(self.sigma) * np.random.rand(self.num_params)  ### 随机生成一个elite
        self.elite_score = None
        ...
        if self.antithetic:
            assert (self.pop_size % 2 == 0), "Population size must be even" ### 如果是偶数，正常运行，否则抛出AssertionError错误，内容为"Population size must be even"
                                                                            ### Python assert（断言）用于判断一个表达式，在表达式条件为 false 的时候触发异常。
        if parents is None or parents <= 0:
            self.parents = pop_size // 2        ### parents个数, 即选fitness高的前1/2个体来更新
        else:
            self.parents = parents
        self.weights = np.array([np.log((self.parents + 1) / i)
                                 for i in range(1, self.parents + 1)])      ### np.log: 以e为底对数, 分数越高，权重越大
        self.weights /= self.weights.sum()
    ...
    def ask(self, pop_size):
        ...
        if self.antithetic and not pop_size % 2:
            ...
            epsilon = np.random.randn(pop_size, self.num_params)    ### 随机给K个个体的参数噪声

        inds = self.mu + self.scale * epsilon * np.sqrt(self.cov)                ### 生成K个个体的参数
        if self.elitism:
            inds[-1] = self.elite
    ...
    def tell(self, solutions, scores):
        ...
        old_mu = self.mu
        self.damp = self.damp * self.tau + (1 - self.tau) * self.damp_limit     ### 噪声指数递减
        self.mu = self.weights @ solutions[idx_sorted[:self.parents]]           ### A@B 矩阵乘法,等于np.dot
        ...
        self.elite = solutions[idx_sorted[0]]
        self.elite_score = scores[idx_sorted[0]]
        # print("self cov:{}\n{}".format(self.cov.shape, self.cov))

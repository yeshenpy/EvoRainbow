...
from EvoRainbow_Exp_core import mod_utils as utils, agent
...
if __name__ == "__main__":
    ...
    while agent.num_frames <= parameters.num_frames:
        ...
        if  stats['log_wandb']:
            ...
            best_old_fitness = stats['best_old_fitness']
            new_fitness = stats['new_fitness']
            final_reward = np.max([RL_reward,erl_score])

            parameters.wandb.log(
                {'elite': elite_rate, 'selected': win_rate, 'discarded': dis_rate,'best_reward': final_reward, 'add_rewards': add_rewards,
        ...
        if agent.num_games > next_save:
            ...
            if elite_index is not None:
                torch.save(agent.pop[elite_index].actor.state_dict(), os.path.join(parameters.save_foldername,
                                                                                   str(parameters.seed) + '_evo_actor_'+str(agent.num_games)+'.pkl'))
